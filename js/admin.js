...
async function handleAdminGenerateCommand() {
    ...
    try {
        ...
        const [pdfUpload, docUpload] = await Promise.all([
            uploadFileToDrive(pdfBlob, `คำสั่ง_${safeCommandId}.pdf`, 'application/pdf', requestData.createdby),
            uploadFileToDrive(docxBlob, `คำสั่ง_${safeCommandId}.docx`, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', requestData.createdby)
        ]);
...
async function adminRegenerateMemoForUser(requestId) {
    ...
    try {
        ...
        const uploadRes = await uploadFileToDrive(pdfBlob, `memo_${safeId}.pdf`, 'application/pdf', req.username || 'admin');
...
async function handleDispatchFormSubmit(e) {
    ...
    try {
        ...
        // --- 4. Upload ไฟล์ขึ้น Google Drive ---
        const uploadResult = await uploadFileToDrive(pdfBlob, `หนังสือส่ง_${requestId.replace(/[\/\\:\.]/g, '-')}.pdf`, 'application/pdf', requestData.createdby);
...
async function handleAdminGenerateMemo() {
    ...
    try {
        ...
        const uploadResult = await uploadFileToDrive(pdfBlob, `บันทึกข้อความ_${requestId.replace(/\//g, '-')}.pdf`, 'application/pdf', requestData.createdby);
...
async function handleAdminMemoActionSubmit(e) {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            // ★ ถ้า GAS ไม่ได้คืน URL → fallback ไปยัง uploader กลางของระบบแทน
            const getOrFallback = async (driveUrl, file, prefix) => {
                if (driveUrl) return driveUrl;
                if (!file) return null;
                try {
                    const res = await uploadFileToDrive(file, `admin_${prefix}_${safeId}_${Date.now()}.pdf`, file.type, 'admin');
                    console.warn(`Google Drive upload (${prefix}) ไม่มี URL → ใช้ uploader กลางสำรอง`);
                    return res?.url || null;
                } catch (err) {
                    console.warn(`Upload fallback (${prefix}) ล้มเหลว:`, err.message);
                    return null;
                }
            };
            ...
            if (typeof db !== 'undefined') {
                 ...
                 // ★ URL Priority: GAS Drive result → allMemosCache → upload fallback
...
async function handleAdminDirectStatusSubmit(e) {
    ...
    try {
        ...
        if (newStatus === 'เสร็จสิ้น/รับไฟล์ไปใช้งาน') {
            ...
            // Fallback: อัปโหลดผ่าน uploader กลางของระบบถ้า GAS ไม่รองรับ uploadAdminFile
            const uploadViaStorage = async (file, prefix) => {
                if (!file) return null;
                const blob = new Blob([file], { type: file.type });
                const res = await uploadFileToDrive(blob, `admin_${prefix}_${safeId}.pdf`, file.type, 'admin');
                return (res.status === 'success' && res.url) ? res.url : null;
            };
...
async function syncUsersToFirebase() {
    if (!checkAdminAccess()) return { status: 'error', message: 'ไม่มีสิทธิ์' };

    try {
        const result = await apiCall('GET', 'getAllUsers');
        if (result.status !== 'success' || !result.data) {
            throw new Error("ไม่สามารถดึงข้อมูลผู้ใช้จาก Google Sheets ได้");
        }
        ...
        console.log(`✅ Synced ${total} users to Firebase`);
        return { status: 'success', total };

    } catch (error) {
        console.error("syncUsersToFirebase Error:", error);
...
// ระบบนี้ถ่ายโอนไฟล์เก่าที่ยังอ้าง Firebase Storage ไปยัง Google Drive
// อัปเดตลิงก์ใน Firestore และ Google Sheets แล้วลบไฟล์เก่าออกจาก Storage

// ฟิลด์ที่อาจยังเก็บ Firebase Storage URL ในแต่ละ record
...
async function previewArchivable() {
    ...
    // นับไฟล์เก่าที่เป็นลิงก์ Firebase Storage และต้องถ่ายโอน
    ...
    fileCount.textContent = `ไฟล์เก่าที่ต้องย้ายเข้า Google Drive: ${filesToArchive.length} ไฟล์`;

    if (filesToArchive.length === 0) {
        previewList.innerHTML = `<p class="text-center text-gray-500 py-4">ไม่พบไฟล์เก่าที่ต้องย้ายในช่วงเวลานี้ (อาจย้ายครบแล้ว หรือยังไม่มีไฟล์)</p>`;
